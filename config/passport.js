...
module.exports = function (passport) {
    ...
    passport.deserializeUser(function (id, done) {
        connection.query("SELECT * FROM users2 WHERE id = ? ", [id], function (err, rows) {
            done(err, rows[0]);
        });
    });
            ...
            function (req, username, password, done) {
                ...
                connection.query("SELECT * FROM users2 WHERE username = ?", [username], function (err, rows) {
                    ...
                    if (rows.length) {
                        ...
                        var insertQuery = "INSERT INTO users2 ( username, password, tipo) values (?,?,?)";

                        connection.query(insertQuery, [newUserMysql.username, newUserMysql.password, req.body.tipo], function (err, rows) {
